    ...
    consentService.getConsentsHistory(this.props.tenant, this.props.organisationKey, this.props.userId, 0, 1)
      .then(currentConsentVersion => {
        ...
        tabs.push({
          ...
          id: 'CURRENT',
          content: <ConsentFactDisplayPage {...this.props} user={currentConsentVersion.items[0]} customLabel={"Version courante de l'utilisateur"}/>,
          deletable: false
        });
        tabs.push({
          title: "Historique",
    ...
    if (tabs.findIndex(tab => tab.id === selectedTab) === -1)
      {
        ...
        tabs.push({
                ...
                id: selectedTab,
                content: <ConsentFactDisplayPage {...this.props} user={user} customLabel={`Aperçu de la version du ${versionDate}`}/>,
                deletable: true
              });
      }
