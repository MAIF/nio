...
class OfferManagerService(
  ...
  def delete(
    ...
    accessibleOfferManagerService.accessibleOfferKey(offerKey, offerRestrictionPatterns) match {
      case true  =>
        updateOrganisation(tenant, orgKey).flatMap {
          case Left(e)  =>
  ...
  private def updateOrganisation(
      tenant: String,
      orgKey: String
    ...
    organisationMongoDataStore.findLastReleasedByKey(tenant, orgKey).flatMap {
      case Some(lastExistingOrganisation) =>
        val oldOrganisation =
          lastExistingOrganisation.copy(version = lastExistingOrganisation.version.copy(latest = false))
        val newOrganisation = oldOrganisation
          .copy(_id = BSONObjectID.generate().stringify, version = oldOrganisation.version.copy(latest = true))
      ...
      case None                           =>
        toErrorWithStatus(s"released.of.organisation.$orgKey.not.found", NotFound)
    }
