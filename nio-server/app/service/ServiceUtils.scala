...
trait ServiceUtils {
  ...
  ): Future[Either[AppErrorWithStatus, T]] =
    toErrorWithStatus(AppErrors(Seq(ErrorMessage(errorMessage))), status)

  protected def toErrorWithStatus[T](appErrors: AppErrors, status: Status): Future[Either[AppErrorWithStatus, T]] =
    toErrorWithStatus(AppErrorWithStatus(appErrors, status))

  protected def toErrorWithStatus[T](appErrors: AppErrorWithStatus): Future[Either[AppErrorWithStatus, T]] =
