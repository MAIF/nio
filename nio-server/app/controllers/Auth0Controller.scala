...
class Auth0Controller(env: Env, wsClient: WSClient, cc: ControllerComponents)(implicit ec: ExecutionContext)
  ...
  def callback(maybeCode: Option[String] = None, maybeState: Option[String] = None) = Action.async { implicit request =>
    NioLogger.info("auth0 callback")
          ...
          .flatMap {
            case Right((idToken, accessToken)) =>
              val future: Future[Result] = getUser(accessToken).map { user =>
                NioLogger.info(s"auth0 callback user $user")
