...
object UploadTracker {
  ...
  def incrementUploadedBytes(appId: String, receivedBytes: Long) = appsUploads.getOrElseUpdate(appId, new LongAdder()).add(receivedBytes)

  def getUploadedBytes(appId: String) = appsUploads.getOrElse(appId, new LongAdder).longValue()
